...
public class KeywordsGame {
    ...
    public void runGame() {
        final  int keyWord = random.nextInt(KEYWORDS.size());
        int incorrectAnswersCounter = 0;
        while (true) {
            final boolean providedAnswer;
            if (keyWord < RESERVED.size()) {
    ...
    private boolean writeReservedQuestion() {
        ...
        final int reserve = random.nextInt(RESERVED.size());
        System.out.println(RESERVED.get(reserve));
        return true;
    }
